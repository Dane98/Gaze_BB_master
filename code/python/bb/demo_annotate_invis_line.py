...
def run(vid_type):
    video_list = vid_type + '_video_list'
    video_folder = vid_type + '_video_folder'
    frame_out_root = vid_type + '_frame_out_root'
    l2cs_root = vid_type + '_l2cs_root'
    ...
    for video in video_list:
        ...
        face_folder = f'./output/face_{vid_type}/'
        ...
        with torch.no_grad():
            ...
            images = [img for img in os.listdir(invis_out_l2cs) if
                      (img.endswith(".jpg") and img.startswith("00"))]
            images = sorted(images)
            # # print(images[0:5])
            frame_out = cv2.imread(os.path.join(invis_out_l2cs, images[0]))
            hh, ww, ll = frame_out.shape
            video = cv2.VideoWriter(video_name, 0, 10, (ww, hh))

            for im in images:
                img_out = cv2.imread(os.path.join(invis_out_l2cs, im))
                video.write(img_out)
            ...
            dataframe.to_csv(root + 'pitchjaw/' + filename + '.csv', index=False)
            ...
            dataframe_filter.to_csv(root + 'pitchjaw/' + filename + '.csv', index=False)

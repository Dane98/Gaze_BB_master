...
def run(vid_type):
    if vid_type == 'vis':
        ...
        l2cs_root = vis_l2cs_root + test_folder
        face_folder = './input/face_baseline/face_vis/'
        result_folder = root + '/pitchjaw/visible_filtered/' + test_folder
        ...
        l2cs_root = invis_l2cs_root + test_folder
        face_folder = './input/face_baseline/face_invis/'
        result_folder = root + '/pitchjaw/invisible_filtered/' + test_folder
    ...
    if not os.path.exists(l2cs_out):
    ...
    # pred_df = pd.read_csv(pred_values_folder + filename + '.csv')

    # pitch_pred_list = pred_df['pitch'].tolist()
    # # print(pitch_pred_list[0:10])
    # # print(len(pitch_pred_list))
    # yaw_pred_list = pred_df['yaw'].tolist()
    #
    # pitch_filtered = klm(np.array(pitch_pred_list))
    # # print(pitch_filtered[0:10])
    # # print(len(pitch_filtered))
    # yaw_filtered = klm(np.array(yaw_pred_list))

    face_l2cs_info = pd.read_csv(os.path.join(face_folder, face_file),
                                 names=['file', 'left', 'top', 'right', 'bottom', 'face'])
    face_base_info = pd.read_csv(os.path.join(face_folder, face_file),
                                 names=['file', 'left', 'top', 'right', 'bottom', 'face'])
    ...
    with torch.no_grad():
        while True:
            ...
            if frame is not None:
                ...
                draw_gaze_extended(x_min_base, y_min_base, bbox_width_base, bbox_height_base, frame,
                                   (pitch_predicted, yaw_predicted), color=(0, 0, 255))  # l2cs output
                cv2.rectangle(frame, (x_min_base, y_min_base), (x_max_base, y_max_base), (0, 255, 0),
                              2)  # using baseline's face
                # (h, w) = frame.shape[:2]
                gaze_class = classify_line_box_intersection(x_min_base, y_min_base, bbox_width_base, bbox_height_base,
                                                            frame, pitch_predicted_[-1], yaw_predicted_[-1],
                                                            face_status,
                                                            vid_type, filename)
                gaze_class_prop = classify_line_box_intersection_proportion(x_min_base, y_min_base, bbox_width_base,
                ...
                # this block is for l2cs to detect the faces in the image, and save the face info into a txt file
                ...
                #     f.write("%05d.jpg" % c)
                #     f.write(',')
                #     f.write(','.join(str(round(float(b))) for b in face_box))
                #     f.write(',')
                ...
                # print(f"myFPS: {myFPS}")
                cv2.putText(frame, 'class: {} class_prop: {} (face: {})'.format(get_classname(gaze_class),
                ...
                cv2.imwrite(l2cs_out_temp + "%05d.jpg" % c, frame) # change here
                c += 1
        ...
        #
        # pitch_pred_list = pred_df['pitch'].tolist()
        # print(pitch_pred_list[0:15])
        # yaw_pred_list = pred_df['yaw'].tolist()

        print(f"Face detection and l2cs prediction for {filename} are done!")
        ...
        # # # print(images[0:5])
        # frame_out = cv2.imread(os.path.join(l2cs_out_temp, images[0]))
        # hh, ww, ll = frame_out.shape
        # video = cv2.VideoWriter('test.avi', 0, 10, (ww, hh))
        #
        # for im in images:
        #     img_out = cv2.imread(os.path.join(l2cs_out_temp, im))
        #     video.write(img_out)
        ...
        for img in images:
            ...
            faces = face_info[face_info['file'] == img].iloc[0]

            if faces['face']:
                face_status = True
                x_min = int(faces['left'])
                y_min = int(faces['top'])
                x_max = int(faces['right'])
                y_max = int(faces['bottom'])
            else:
                face_status = False
            ...
            # #     gaze_class_f = 4
            # else:
            draw_gaze_norm(x_min, y_min, bbox_width, bbox_height, frame,
                           (pitch_filtered[count], yaw_filtered[count]), color=(0, 255, 0))
            draw_gaze_extended(x_min, y_min, bbox_width, bbox_height, frame,
                               (pitch_filtered[count], yaw_filtered[count]), color=(0, 255, 0))
            # cv2.rectangle(frame, (x_min, y_min), (x_max, y_max), (0, 255, 0), 1)  # using l2cs' face
            gaze_class_f = classify_line_box_intersection(x_min, y_min, bbox_width, bbox_height, frame,
                                                          pitch_filtered[count], yaw_filtered[count], face_status,
                                                          vid_type, filename)
            gaze_class_prop_f = classify_line_box_intersection_proportion(x_min, y_min, bbox_width, bbox_height, frame,
                                                          pitch_filtered[count], yaw_filtered[count], face_status,
                                                          vid_type, filename)
            ...
            # myFPS = 1.0 / (time.time() - start_fps)
            cv2.putText(frame, '(filtered_class: {} filt_prop_cls: {})'.format(get_classname(gaze_class_f),
                                                                             get_classname(gaze_class_prop_f)),
                        (10, 50), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 255, 0), 1, cv2.LINE_AA)

            cv2.imshow("Demo", frame)
            # cv2.imwrite(invis_root + filename + "/" + "%05d.jpg" % count, frame)
            cv2.imwrite(l2cs_out + "%05d.jpg" % count, frame)  # change here
            count += 1
            if cv2.waitKey(1) & 0xFF == 27:
                print('stuck here')
                break
        ...
        # dataframe.to_csv(root + result_folder + filename + '.csv', index=False)
        ...
        dataframe_filter = pd.DataFrame(
            data=np.concatenate(
                [np.array(pitch_filtered, ndmin=2), np.array(yaw_filtered, ndmin=2),
                 ...
                 np.array(gaze_class_filtered, ndmin=2), np.array(gaze_class_prop_filtered, ndmin=2)]).T,
            columns=["yaw", "pitch", 'm2_class', 'm2prop_class', 'filtered_m2_class', 'filtered_prop_class'])
        dataframe_filter.to_csv(result_folder + filename + '.csv', index=False) # change here
...
if __name__ == '__main__':
    ...
    ext = '.MP4'
    # filename = '33002_sessie2_taskrobotEngagement'    # vis
    # filename = '51007_sessie1_taskrobotEngagement'    # invis
    filename = '51017_sessie3_taskrobotEngagement'    # invis

    # ext = '.mp4'
    # filename = 'Proefpersoon22016_Sessie1'  # vis
    # filename = 'Proefpersoon11029_sessie2'  # invis
    file = filename + ext

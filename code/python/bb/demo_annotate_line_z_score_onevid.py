...
def run(vid_type):
    ...
    face_l2cs_info = pd.read_csv(os.path.join(f'./output/face_{vid_type}/', face_file), names=['file', 'left', 'top', 'right', 'bottom', 'face'])
    face_base_info = pd.read_csv(os.path.join(f'./input/face_baseline/face_{vid_type}/', face_file), names=['file', 'left', 'top', 'right', 'bottom', 'face'])
    ...
    with torch.no_grad():
        while True:
            ...
            if frame is not None:
                ...
                faces_base = face_base_info[face_base_info['file'] == img].iloc[0]
                print(f'baseline_face: img = {img}')
                if faces_base['face']:
                ...
                if len(faces) > 0:
                    ...
                    draw_gaze(x_min_base, y_min, bbox_width, bbox_height, frame,
                              (pitch_predicted, yaw_predicted), color=(0, 0, 255)) # l2cs output
                    cv2.rectangle(frame, (x_min_base, y_min_base), (x_max_base, y_max_base), (0, 255, 0), 1) # using baseline's face
                    # (h, w) = frame.shape[:2]
                    gaze_class = classify_line_box_intersection(x_min, y_min, bbox_width, bbox_height, frame,
                                                                pitch_predicted_[-1], yaw_predicted_[-1], vid_type,
                                                                filename)
                else:
                    face_box = [0, 0, 0, 0]
                ...
                # print(f"myFPS: {myFPS}")
                cv2.putText(frame, 'pred_class {} (face: {} FPS: {:.1f})'.format(get_classname(gaze_class), face_status,
                                                                                 myFPS), (10, 20),
                            cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 255, 0), 1, cv2.LINE_AA)
        ...
        for img in images:
            ...
            # gaze_class_f = classify_line_box_intersection(0, 0, 0, 0, frame, pitch_filtered[count],
            ...
            # myFPS = 1.0 / (time.time() - start_fps)
            cv2.putText(frame, '(filtered_class {})'.format(get_classname(gaze_class_f)), (10, 50),
                        cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 255, 0), 1, cv2.LINE_AA)
        ...
        dataframe_filter = pd.DataFrame(
            data=np.concatenate(
                [np.array(pitch_filtered, ndmin=2), np.array(yaw_filtered, ndmin=2),
                 np.array(gaze_class_list, ndmin=2), np.array(gaze_class_filtered, ndmin=2)]).T,
            columns=["yaw", "pitch", 'pred_class', 'filtered_class'])
        dataframe_filter.to_csv(result_folder + filename + '_filtered.csv', index=False)
